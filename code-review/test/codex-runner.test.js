...
test("injects local skill files into the Codex prompt and logs which files were used", () => {
  ...
  try {
    ...
    writeFileSync(join(skillDir, "SKILL.md"), "# Code Judgment\n\nUse direct correctness judgment.");
    writeFileSync(join(skillDir, "principles", "indirection.md"), "# Indirection\n\nAvoid needless wrappers.");
    writeFileSync(
      commandPath,
      [
        "#!/usr/bin/env node",
    ...
    assert.deepEqual(JSON.parse(readFileSync(join(eventLogDir, "skill-context.json"), "utf8")), {
